...
pypa_synch_dev_log_handler = handlers.TimedRotatingFileHandler(
...
pypa_synch_dev_log_handler.setFormatter(_dev_formatter)
pypa_synch_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

pypa_synch_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

pypa_synch_usr_log_handler.setLevel(logging.INFO)
...
pypa_synch_usr_log_handler.setFormatter(_usr_formatter)
...
pypa_synch_console_log_handler = logging.StreamHandler(sys.stdout)
pypa_synch_console_log_handler.setFormatter(_console_formatter)
pypa_synch_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    pypa_synch_usr_log_handler.addFilter(warning_filter)
    pypa_synch_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    pypa_synch_usr_log_handler.removeFilter(warning_filter)
    pypa_synch_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    pypa_synch_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(pypa_synch_dev_log_handler)

    log.addHandler(pypa_synch_console_log_handler)

    log.addHandler(pypa_synch_usr_log_handler)
